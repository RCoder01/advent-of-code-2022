...
fn main_0(input: &mut BufReader<File>) -> Result<(), std::io::Error> {
    ...
    for line in input.lines() {
        ...
        let char = comp_1
            ...
            .into_iter()
            .next()
            .expect("Rucksack compartments share no element");
        priority += match char {
            b'A'..=b'Z' => **char - 64 + 26,
            b'a'..=b'z' => **char - 96,
            _ => 0,
    ...
    loop {
        if lines.len() >= 3 {
            let char = lines
                ...
                .fold(BTreeSet::new(), |accum, item| {
                    if accum.is_empty() {
                        item
                    } else {
                        accum.intersection(&item).into_iter().cloned().collect()
                    }
                })
                .into_iter()
                .next()
                .expect("Sequence contains no common character");
            priority += match char {
                b'A'..=b'Z' => char - 64 + 26,
                b'a'..=b'z' => char - 96,
                _ => 0,
